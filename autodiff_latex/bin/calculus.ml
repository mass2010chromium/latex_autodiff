...
let rec derivative node ctx varname =
  ...
  | Unary (U_COS, n) -> let%bind v = derivative n ctx varname in Ok (Binop (B_MUL, ((Binop (B_MUL, (Leaf_NUM (-1.), Unary (U_SIN, n)))), v)))
  | Unary (U_SIN, n) -> let%bind v = derivative n ctx varname in Ok (Binop (B_MUL, ((Unary (U_COS, n)), v)))
  | Unary (U_LOG, n) -> let%bind v = derivative n ctx varname in Ok (Binop (B_MUL, ((Binop (B_EXP, (n, (Leaf_NUM (-1.))))), v)))
